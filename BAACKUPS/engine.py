...
def start(decider):
    ...
    try:
        ...
        if decider == 1: # Multiple files
            ...
            h = [str(hashlib.md5(open(a,'rb').read()).hexdigest()) for a in filenames]
            return [h,filename]
...
def main(h,filename):
    ...
    while num > -1:
        ...
        #num = num -1
        if str( a := requests.get('https://www.virustotal.com/vtapi/v2/file/report', params=params, headers=headers)) == '<Response [204]>'  and file_counter >0:
            ...
            eli_count = 0
            success = False
            while str( a := requests.get('https://www.virustotal.com/vtapi/v2/file/report', params=params, headers=headers)) == '<Response [204]>':
            ...
            d = response.json()
        elif str( requests.get('https://www.virustotal.com/vtapi/v2/file/report', params=params, headers=headers)) == '<Response [200]>'  :    
            response = requests.get('https://www.virustotal.com/vtapi/v2/file/report',
            params=params, headers=headers)
            d = response.json()
            #print(d)
